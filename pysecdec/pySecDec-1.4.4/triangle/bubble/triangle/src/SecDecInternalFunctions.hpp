#ifndef triangle_src_SecDecInternalFunctions_hpp_included
#define triangle_src_SecDecInternalFunctions_hpp_included
...
namespace triangle
{
    ...
    #define triangle_has_complex_parameters 0
    #define triangle_enforce_complex_return_type 0
    #if triangle_has_complex_parameters || triangle_contour_deformation || triangle_enforce_complex_return_type
      #ifdef SECDEC_WITH_CUDA
        __host__ __device__
      #endif
        ...
        inline real_t log(real_t arg)
        {
            if (arg < 0)
            {
                    ...
                    error_message += "Encountered \"log(<negative real>)\" in a real-valued integrand function of \"triangle\". ";
    ...
    complex_t inline pow(complex_t x, int y)
    {
        ...
        inline real_t pow(real_t base, real_t exponent)
        {
            if (base < 0)
            {
                    ...
                    error_message += "\"triangle\". Try to enforce complex return values for the generated integrands; i.e. set ";
                    error_message += "\"enforce_complex=True\" in the corresponding call to \"loop_package\" or \"make_package\".";
                    throw std::domain_error(error_message);
                #endif
            }
    ...
    #undef triangle_has_complex_parameters
    #undef triangle_enforce_complex_return_type
