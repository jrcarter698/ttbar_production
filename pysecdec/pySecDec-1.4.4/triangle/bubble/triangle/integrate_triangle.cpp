...
int main()
{
    ...
    const std::vector<triangle::nested_series_t<triangle::integrand_t>> sector_integrands = triangle::make_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    const triangle::nested_series_t<triangle::integrand_t> all_sectors = std::accumulate(++sector_integrands.begin(), sector_integrands.end(), *sector_integrands.begin());

    // Integrate
    secdecutil::cuba::Vegas<triangle::integrand_return_t> integrator;
