...
int main()
{
    ...
    const std::vector<banana_3mass::nested_series_t<banana_3mass::cuda_integrand_t>> sector_integrands = banana_3mass::make_cuda_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    const banana_3mass::nested_series_t<banana_3mass::cuda_together_integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), banana_3mass::cuda_together_integrand_t()+*sector_integrands.begin());

    // Integrate
    secdecutil::integrators::Qmc<
                                    banana_3mass::integrand_return_t,
                                    banana_3mass::maximal_number_of_integration_variables,
                                    integrators::transforms::/* EDIT: insert transform here */::type,
                                    banana_3mass::cuda_together_integrand_t
                                > integrator;
