...
int main()
{
    ...
    const std::vector<bubble6L::nested_series_t<bubble6L::integrand_t>> sector_integrands = bubble6L::make_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    const bubble6L::nested_series_t<bubble6L::integrand_t> all_sectors = std::accumulate(++sector_integrands.begin(), sector_integrands.end(), *sector_integrands.begin());

    // Integrate
    secdecutil::cuba::Vegas<bubble6L::integrand_return_t> integrator;
