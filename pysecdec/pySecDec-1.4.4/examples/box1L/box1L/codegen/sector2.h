...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (-1))^(-1)))*eps^-1) * ( + (((( + (1))^( + (1))) * (( + ( + (1)))^( + (0))) * (( + ( + (1)))^( + (0) + (-2))) * (( + (1))^( + (1)))) * ((SecDecInternalCalI( + (1)*x0, + (1)*x1, + (0), + (0))))) + (((((( + (1)) * (dSecDecInternalCalId3( + (1)*x0, + (1)*x1, + (0), + (0)))) * ( + (1))))) * ( + (1)))*eps)) + (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1)*x2)^( + (1))) * (( + ( + (1)))^( + (0))) * (( + ( + (1))*x2)^( + (0) + (-2))) * (( + (1))^( + (1)))) * ((SecDecInternalCalI( + (1)*x0, + (1)*x1, + (1)*x2, + (0))) + (( + (-1)) * (SecDecInternalCalI( + (1)*x0, + (1)*x1, + (0), + (0))))))));
...
#procedure insertOther
  ...
  Id SecDecInternalCondefFac(x0?,x1?,x2?,eps?) = ((SecDecInternalCondefFacx2( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-1) + (-1)*eps));
  Id dSecDecInternalCondefFacd3(x0?,x1?,x2?,eps?) = (((SecDecInternalCondefFacx2( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-1) + (-1)*eps)) * ( + (-1)) * (log(SecDecInternalCondefFacx2( + (1)*x0, + (1)*x1, + (1)*x2)))) + (((SecDecInternalCondefFacx2( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-2) + (-1)*eps)) * ( + (0)));
...
#procedure insertDecomposed
  ...
  Id dFd1(x0?,x1?,x2?,eps?) =  + (msq - t)*x0;
  Id F(x0?,x1?,x2?,eps?) =  + ( + (-s)) + ( + (msq - s1))*x0*x2 + ( + (msq - t))*x0*x1 + ( + (msq))*x0 + ( + (msq))*x0^2*x2;
  Id dFd2(x0?,x1?,x2?,eps?) =  + (msq - s1)*x0 + (msq)*x0^2;
  Id U(x0?,x1?,x2?,eps?) =  + ( + (1))*x0*x2 + ( + (1)) + ( + (1))*x1 + ( + (1))*x2;
  Id ddFd0d0(x0?,x1?,x2?,eps?) =  + (2*msq)*x2;
  Id ddFd0d1(x0?,x1?,x2?,eps?) =  + (msq - t);
  Id ddFd1d1(x0?,x1?,x2?,eps?) =  + (0);
  Id dFd0(x0?,x1?,x2?,eps?) =  + (msq) + (msq - s1)*x2 + (msq - t)*x1 + (2*msq)*x0*x2;
  Id ddFd0d2(x0?,x1?,x2?,eps?) =  + (msq - s1) + (2*msq)*x0;
...
#define integrandContainerInitializer "{-1,0,{{2,\{-1\},sector_2_order_n1_numIV,sector_2_order_n1_integrand,#@SecDecInternalNewline@##ifdef#@SecDecInternalSpace@#SECDEC_WITH_CUDA#@SecDecInternalNewline@#get_device_sector_2_order_n1_integrand,#@SecDecInternalNewline@##endif#@SecDecInternalNewline@#sector_2_order_n1_contour_deformation_polynomial,sector_2_order_n1_maximal_allowed_deformation_parameters},{2,\{0\},sector_2_order_0_numIV,sector_2_order_0_integrand,#@SecDecInternalNewline@##ifdef#@SecDecInternalSpace@#SECDEC_WITH_CUDA#@SecDecInternalNewline@#get_device_sector_2_order_0_integrand,#@SecDecInternalNewline@##endif#@SecDecInternalNewline@#sector_2_order_0_contour_deformation_polynomial,sector_2_order_0_maximal_allowed_deformation_parameters}},true,#@SecDecInternalDblquote@#eps#@SecDecInternalDblquote@#}"
