...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (-1))^(-1)))*eps^-1) * ( + (((( + (1))^( + (1))) * (( + ( + (1)))^( + (0))) * (( + ( + (1)))^( + (0) + (-2))) * (( + (1))^( + (1)))) * ((SecDecInternalCalI( + (0), + (1)*x1, + (1)*x2, + (0))))) + (((((( + (1)) * (dSecDecInternalCalId3( + (0), + (1)*x1, + (1)*x2, + (0)))) * ( + (1))))) * ( + (1)))*eps)) + (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1)*x0)^( + (1))) * (( + ( + (1)))^( + (0))) * (( + ( + (1))*x0)^( + (0) + (-2))) * (( + (1))^( + (1)))) * ((SecDecInternalCalI( + (1)*x0, + (1)*x1, + (1)*x2, + (0))) + (( + (-1)) * (SecDecInternalCalI( + (0), + (1)*x1, + (1)*x2, + (0))))))));
...
#procedure insertOther
  ...
  Id SecDecInternalCondefFac(x0?,x1?,x2?,eps?) = ((SecDecInternalCondefFacx0( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-1) + (-1)*eps));
  Id dSecDecInternalCondefFacd3(x0?,x1?,x2?,eps?) = (((SecDecInternalCondefFacx0( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-1) + (-1)*eps)) * ( + (-1)) * (log(SecDecInternalCondefFacx0( + (1)*x0, + (1)*x1, + (1)*x2)))) + (((SecDecInternalCondefFacx0( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-2) + (-1)*eps)) * ( + (0)));
...
#procedure insertDecomposed
  ...
  Id dFd1(x0?,x1?,x2?,eps?) =  + (-s)*x2 + (msq)*x0;
  Id F(x0?,x1?,x2?,eps?) =  + ( + (-s))*x1*x2 + ( + (msq - s1))*x2 + ( + (msq - t)) + ( + (msq))*x0*x1 + ( + (msq))*x0;
  Id dFd2(x0?,x1?,x2?,eps?) =  + (msq - s1) + (-s)*x1;
  Id U(x0?,x1?,x2?,eps?) =  + ( + (1))*x0 + ( + (1))*x0*x1 + ( + (1)) + ( + (1))*x2;
  Id ddFd0d0(x0?,x1?,x2?,eps?) =  + (0);
  Id ddFd0d1(x0?,x1?,x2?,eps?) =  + (msq);
  Id ddFd1d1(x0?,x1?,x2?,eps?) =  + (0);
  Id dFd0(x0?,x1?,x2?,eps?) =  + (msq) + (msq)*x1;
  Id ddFd0d2(x0?,x1?,x2?,eps?) =  + (0);
  Id ddFd1d2(x0?,x1?,x2?,eps?) =  + (-s);
...
#define integrandContainerInitializer "{-1,0,{{7,\{-1\},sector_7_order_n1_numIV,sector_7_order_n1_integrand,#@SecDecInternalNewline@##ifdef#@SecDecInternalSpace@#SECDEC_WITH_CUDA#@SecDecInternalNewline@#get_device_sector_7_order_n1_integrand,#@SecDecInternalNewline@##endif#@SecDecInternalNewline@#sector_7_order_n1_contour_deformation_polynomial,sector_7_order_n1_maximal_allowed_deformation_parameters},{7,\{0\},sector_7_order_0_numIV,sector_7_order_0_integrand,#@SecDecInternalNewline@##ifdef#@SecDecInternalSpace@#SECDEC_WITH_CUDA#@SecDecInternalNewline@#get_device_sector_7_order_0_integrand,#@SecDecInternalNewline@##endif#@SecDecInternalNewline@#sector_7_order_0_contour_deformation_polynomial,sector_7_order_0_maximal_allowed_deformation_parameters}},true,#@SecDecInternalDblquote@#eps#@SecDecInternalDblquote@#}"
