...
nested_series_t<sector_container_t> get_integrand_of_sector_8();
    ...
    {
        if ( real_parameters.size() != box1L::number_of_real_parameters )
            throw std::logic_error(
                                        "Called \"box1L::make_integrands\" with " +
                                        std::to_string(real_parameters.size()) + " \"real_parameters\" (" +
                                        std::to_string(box1L::number_of_real_parameters) + " expected)."
                                  );

        if ( complex_parameters.size() != box1L::number_of_complex_parameters )
            throw std::logic_error(
                                        "Called \"box1L::make_integrands\" with " +
                                        std::to_string(complex_parameters.size()) + " \"complex_parameters\" (" +
                                        std::to_string(box1L::number_of_complex_parameters) + " expected)."
                                  );
    };


    #define box1L_contour_deformation 1
    ...
    )
    {
        check_parameter_sizes(real_parameters, complex_parameters);
            ...
            std::vector<nested_series_t<
                ...
                maximal_number_of_integration_variables,number_of_real_parameters,number_of_complex_parameters,'b','o','x','1','L'>
        ...
        #else
            std::vector<nested_series_t<
            secdecutil::CudaIntegrandContainerWithoutDeformation<real_t,complex_t,integrand_return_t,1/*maximal_number_of_functions*/,number_of_real_parameters,number_of_complex_parameters,'b','o','x','1','L'>
            >> make_cuda_integrands
            (
                const std::vector<real_t>& real_parameters,
                const std::vector<complex_t>& complex_parameters
    ...
    #undef box1L_contour_deformation
};
