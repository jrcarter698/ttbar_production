...
int main()
{
    ...
    const std::vector<bubble2l::nested_series_t<bubble2l::cuda_integrand_t>> sector_integrands = bubble2l::make_cuda_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    const bubble2l::nested_series_t<bubble2l::cuda_together_integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), bubble2l::cuda_together_integrand_t()+*sector_integrands.begin());

    // Integrate
    secdecutil::integrators::Qmc<
                                    bubble2l::integrand_return_t,
                                    bubble2l::maximal_number_of_integration_variables,
                                    integrators::transforms::/* EDIT: insert transform here */::type,
                                    bubble2l::cuda_together_integrand_t
                                > integrator;
