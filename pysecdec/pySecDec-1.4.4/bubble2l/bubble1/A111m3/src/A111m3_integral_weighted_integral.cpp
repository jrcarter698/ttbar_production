...
#include "A111m3_integral_weighted_integral.hpp"

#define INTEGRAL_NAME A111m3
...
namespace A111m3_integral
{
    ...
    {
        assert(A111m3::requested_orders.size() == A111m3_integral::requested_orders.size());
        size_t number_of_regulators = A111m3::requested_orders.size();
        std::vector<int> orders; orders.reserve( A111m3::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              A111m3::requested_orders.at(i) + 1
                            - A111m3_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - A111m3_integral::lowest_orders.at(i)
                            - A111m3_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/A111m3_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace A111m3
{
    namespace A111m3_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::A111m3_integral::integrand_t;

            // Note: we define make_integrands with A111m3_contour_deformation
            // but call ::sub_integral_name::make_integrands with A111m3_integral_contour_deformation
            ...
            {
                    return ::A111m3_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::A111m3_integral::cuda_integrand_t;
                ...
                {
                        return ::A111m3_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if A111m3_integral_contour_deformation
        ...
        // and return one with ::A111m3_integral::(cuda_)integrand_t instead
        #define INSTANTIATE_AMPLITUDE_INTEGRAL_NONE_QMC() \
            ...
            { \
                using amplitude_integrator_t = secdecutil::integrators::Qmc< \
                     ...
                     INTEGRAL_NAME::maximal_number_of_integration_variables, \
                     ::integrators::transforms::None::type, \
                     ::A111m3_integral::INTEGRAND_TYPE, \
                     secdecutil::integrators::void_template \
                >; \
        ...
        )
        {
            ...
            using integrand_return_t = A111m3::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::A111m3_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::A111m3_integral::prefactor(real_parameters,complex_parameters)
                       * ::A111m3_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
