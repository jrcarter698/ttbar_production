...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<A111m3_integral::nested_series_t<A111m3_integral::integrand_t>> sector_integrands =
        A111m3_integral::make_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    std::cerr << "Summing integrands" << std::endl;
    const A111m3_integral::nested_series_t<A111m3_integral::integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), *sector_integrands.begin());

    // Integrate
    std::cerr << "Integrating" << std::endl;
    secdecutil::cuba::Vegas<A111m3_integral::integrand_return_t> integrator;
