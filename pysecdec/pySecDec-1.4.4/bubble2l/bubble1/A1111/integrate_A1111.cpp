...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 + 3 + 2*0) {
        ...
        for ( const auto& name : A1111::names_of_real_parameters )
            std::cout << " " << name;
        for ( const auto& name : A1111::names_of_complex_parameters )
            std::cout << " re(" << name << ") im(" << name << ")";
        std::cout << std::endl;
        return 1;
    }

    std::vector<A1111::real_t> real_parameters; // = { real parameter values ("s","t","m2") go here };
    std::vector<A1111::complex_t> complex_parameters; // = { complex parameter values () go here };

    // Load parameters from the command line arguments
    for (int i = 1; i < 1 + 3; i++)
        real_parameters.push_back(A1111::real_t(std::atof(argv[i])));

    for (int i = 1 + 3; i < 1 + 3 + 2*0; i += 2) {
        A1111::real_t re = std::atof(argv[i]);
        A1111::real_t im = std::atof(argv[i+1]);
        complex_parameters.push_back(A1111::complex_t(re, im));
    }

    // Set up Integrator
    std::cerr << "Setting up integrator" << std::endl;
    //secdecutil::cuba::Vegas<A1111::integrand_return_t> integrator;
    secdecutil::integrators::Qmc<
                                    A1111::integrand_return_t,
                                    A1111::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    A1111::user_integrand_t
                                > integrator;
    integrator.verbosity = 1;

    // Construct the amplitudes
    std::cerr << "Generating amplitudes (optimising contour if required)" << std::endl;
    std::vector<A1111::nested_series_t<A1111::sum_t>> unwrapped_amplitudes =
        A1111::make_amplitudes(real_parameters, complex_parameters, "A1111_data", integrator);

    // Pack amplitudes into handler
    std::cerr << "Packing amplitudes into handler" << std::endl;
    A1111::handler_t<A1111::amplitudes_t> amplitudes
    (
    ...
    const std::vector<A1111::nested_series_t<secdecutil::UncorrelatedDeviation<A1111::integrand_return_t>>> result = amplitudes.evaluate();

    // print the result
    for (unsigned int amp_idx = 0; amp_idx < A1111::number_of_amplitudes; ++amp_idx)
