...
#include "A1111_integral_weighted_integral.hpp"

#define INTEGRAL_NAME A1111
...
namespace A1111_integral
{
    ...
    {
        assert(A1111::requested_orders.size() == A1111_integral::requested_orders.size());
        size_t number_of_regulators = A1111::requested_orders.size();
        std::vector<int> orders; orders.reserve( A1111::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              A1111::requested_orders.at(i) + 1
                            - A1111_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - A1111_integral::lowest_orders.at(i)
                            - A1111_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/A1111_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace A1111
{
    namespace A1111_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::A1111_integral::integrand_t;

            // Note: we define make_integrands with A1111_contour_deformation
            // but call ::sub_integral_name::make_integrands with A1111_integral_contour_deformation
            ...
            {
                    return ::A1111_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::A1111_integral::cuda_integrand_t;
                ...
                {
                        return ::A1111_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if A1111_integral_contour_deformation
        ...
        // and return one with ::A1111_integral::(cuda_)integrand_t instead
        ...
        )
        {
            ...
            using integrand_return_t = A1111::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::A1111_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::A1111_integral::prefactor(real_parameters,complex_parameters)
                       * ::A1111_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
