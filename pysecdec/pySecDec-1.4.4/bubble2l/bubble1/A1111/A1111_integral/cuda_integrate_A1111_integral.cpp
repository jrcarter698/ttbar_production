...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<A1111_integral::nested_series_t<A1111_integral::cuda_integrand_t>> sector_integrands =
        A1111_integral::make_cuda_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    std::cerr << "Summing integrands" << std::endl;
    const A1111_integral::nested_series_t<A1111_integral::cuda_together_integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), A1111_integral::cuda_together_integrand_t()+*sector_integrands.begin());

    // Integrate
    std::cerr << "Integrating" << std::endl;
    secdecutil::integrators::Qmc<
                                    A1111_integral::integrand_return_t,
                                    A1111_integral::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    A1111_integral::cuda_together_integrand_t
                                > integrator;
