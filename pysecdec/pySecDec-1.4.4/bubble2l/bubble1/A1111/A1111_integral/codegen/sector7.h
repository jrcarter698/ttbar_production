...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1)*x1^2*x2^4)^( + (1))) * (( + ( + (1)))^( + (0) + (1))) * (( + ( + (1))*x1*x2^2)^( + (0) + (-3))) * (( + ( + (1))*x1*x2^2)^( + (1))) * (( + (1))^( + (1)))) * (SecDecInternalCalI( + (1)*x0, + (1)*x1, + (1)*x2, + (0))))));
...
#procedure insertOther
  ...
  Id SecDecInternalCondefFac(x0?,x1?,x2?,eps?) = ((SecDecInternalCondefFacx0( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (0))) * ((SecDecInternalCondefFacx1( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-1)*eps)) * ((SecDecInternalCondefFacx2( + (1)*x0, + (1)*x1, + (1)*x2)) ^ ( + (-2)*eps));
  Id SecDecInternalOtherPoly0(x0?,x1?,x2?,eps?) =  + ( + (1));

#endProcedure

#procedure insertDecomposed
    Id ddFd0d0(x0?,x1?,x2?,eps?) =  + (2*m2)*x1*x2^2;
  Id dFd1(x0?,x1?,x2?,eps?) =  + (m2) + (m2)*x2 + (m2)*x2^2 + (2*m2)*x0*x2 + (2*m2)*x0*x2^2 + (m2)*x0^2*x2^2;
  Id F(x0?,x1?,x2?,eps?) =  + ( + (m2))*x1*x2^2 + ( + (m2)) + ( + (m2))*x1*x2 + ( + (m2))*x1 + ( + (2*m2))*x0*x1*x2^2 + ( + (m2))*x0 + ( + (2*m2))*x0*x1*x2 + ( + (m2))*x0^2*x1*x2^2;
  Id ddFd0d1(x0?,x1?,x2?,eps?) =  + (2*m2)*x2 + (2*m2)*x2^2 + (2*m2)*x0*x2^2;
  Id U(x0?,x1?,x2?,eps?) =  + ( + (1))*x0*x1*x2^2 + ( + (1))*x1*x2 + ( + (1)) + ( + (1))*x1*x2^2;
  Id dFd2(x0?,x1?,x2?,eps?) =  + (m2)*x1 + (2*m2)*x1*x2 + (2*m2)*x0*x1 + (4*m2)*x0*x1*x2 + (2*m2)*x0^2*x1*x2;
  Id ddFd1d2(x0?,x1?,x2?,eps?) =  + (m2) + (2*m2)*x2 + (2*m2)*x0 + (4*m2)*x0*x2 + (2*m2)*x0^2*x2;
  Id dFd0(x0?,x1?,x2?,eps?) =  + (m2) + (2*m2)*x1*x2 + (2*m2)*x1*x2^2 + (2*m2)*x0*x1*x2^2;
  Id ddFd2d2(x0?,x1?,x2?,eps?) =  + (2*m2)*x1 + (4*m2)*x0*x1 + (2*m2)*x0^2*x1;
  Id ddFd1d1(x0?,x1?,x2?,eps?) =  + (0);
  Id SecDecInternalDecoPoly2(x0?,x1?,x2?,eps?) =  + ( + (1));
  Id ddFd0d2(x0?,x1?,x2?,eps?) =  + (2*m2)*x1 + (4*m2)*x1*x2 + (4*m2)*x0*x1*x2;
