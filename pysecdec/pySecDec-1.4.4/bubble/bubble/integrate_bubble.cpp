...
int main()
{
    ...
    const std::vector<bubble::nested_series_t<bubble::integrand_t>> sector_integrands = bubble::make_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    const bubble::nested_series_t<bubble::integrand_t> all_sectors = std::accumulate(++sector_integrands.begin(), sector_integrands.end(), *sector_integrands.begin());

    // Integrate
    secdecutil::cuba::Vegas<bubble::integrand_return_t> integrator;
